...
impl<T, A: Allocator> Vec<T, A> {
    ...
    /// Resizes the vector to `new_len`, using `f` to produce new elements.
    ///
    /// If `new_len` is less than the current length, the vector is truncated.
    /// If greater, new elements are generated by calling `f()` for each slot.
    pub fn resize_with(
        &mut self,
        new_len: usize,
        flags: Flags,
        ...
        while self.len < new_len {
            // SAFETY: reserve guarantees capacity.
            unsafe {
                self.ptr.as_ptr().add(self.len).write(f());
            }
            self.len += 1;
        }
        Ok(())
...
impl<T: Clone, A: Allocator> Vec<T, A> {
    ...
    ///
    /// If `new_len` is less than the current length, the vector is truncated.
    /// If greater, new elements are cloned from `value`.
    pub fn resize(&mut self, new_len: usize, value: T, flags: Flags) -> Result<(), AllocError> {
        ...
        self.reserve(new_len - self.len, flags)?;
        while self.len < new_len {
            // SAFETY: reserve guarantees capacity; push won't realloc.
            unsafe {
                self.ptr.as_ptr().add(self.len).write(value.clone());
            }
            self.len += 1;
        }
        Ok(())
