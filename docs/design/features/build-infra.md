...
**Samples cannot be workspace members.** The kernel build requires:

- `build-std = ["core"]` — recompiles `core` for the kernel target
- A custom `target.json` (kernel target triple)
- Kernel-specific rustflags (`-Ccode-model=kernel`, `-Cno-redzone`, etc.)

These are passed via `cargo --config cargo-kernel.toml --config 'build.target=...'`
and only apply to that specific cargo invocation. If samples were workspace
members, `cargo check` / `cargo clippy` on the host would either:

- Fail to find the kernel target JSON (it's a path in `linux_bin/`)
- Apply kernel rustflags to host builds (breaking compilation)
- Try to build-std core for the host (unnecessary and slow)

Cargo has no per-member target or rustflags scoping. The `--config` flag
is invocation-scoped, not member-scoped.

**Verdict**: keep samples in `exclude`, use `--config` at build time.
...
Each sample's `Cargo.toml` is minimal:
...
**After** (2 files, no cmake/script changes):
```
samples/new_module/
├── Cargo.toml            # minimal: name + deps
└── new_module.rs         # actual source
CMakeLists.txt            # add 1 line: add_kernel_module(NAME new_module ...)
```
